if (window.__OVERTIME_TRACKER_SCRIPT_LOADED__) {
  ...
  (() => {
      ...
      let records = [];
      let currentFilteredRecords = []; // Currently visible records on the table
      let editingId = null;
      let isInitialLoad = true;
      let isLoginMode = true; 
      
      // ──────────────────────────────────────────
      // ২. AUTHENTICATION & PROFILE LOGIC
      // ──────────────────────────────────────────
      ...
      async function handleAuth() {
        ...
        try {
          if (isLoginMode) {
            ...
            if (error) throw error;
            authError.textContent = 'Signup successful! You can now login.'; authError.style.color = '#22c55e';
          }
        } catch (err) {
          authError.textContent = err.message || 'An error occurred.'; authError.style.color = '#ef4444';
        }
      }
      ...
      // Update UI Function for Profile
      ...
      document.getElementById('editProfileForm').addEventListener('submit', async (e) => {
          ...
          const meta = {
              ...
              fullName: document.getElementById('editFull').value,
              designation: document.getElementById('editDesig').value,
              department: document.getElementById('editDept').value,
              institution: document.getElementById('editInst').value,
              contact: document.getElementById('editContact').value,
              officeStart: document.getElementById('editStart').value,
              officeEnd: document.getElementById('editEnd').value
          };
          const { data, error } = await supabase.auth.updateUser({ data: meta });
          if(!error) {
             currentUser = data.user;
             updateProfileUI();
             filterRecords(); // Recalculate OT if office time changed
             document.getElementById('editProfileModal').style.display = 'none';
          } else {
      ...
      async function handleFormSubmit(e) {
        ...
        const record = {
          ...
          clockIn: document.getElementById('clockIn').value, clockOut: document.getElementById('clockOut').value,
      ...
      document.getElementById('clearBtn').addEventListener('click', () => {
        document.getElementById('otForm').reset(); document.getElementById('dayName').value = ''; editingId = null;
      });
      ...
      // Filtered Excel Export logic (Fixed)
      ...
      if(exportExcelBtn) {
        exportExcelBtn.addEventListener('click', () => {
          ...
          // Only use the currently filtered data!
          currentFilteredRecords.forEach(r => {
            const ot = calculateOT(r);
            grandTotalMinutesForExcel += ot.total;
          ...
          // Auto-adjust column width for better look
          ...
          reader.onload = async function(e) {
            try {
              ...
              for (let row of jsonData) {
                ...
                if(!rawDate || rawDate === 'GRAND TOTAL') continue;
      
                let isoDate = rawDate;
                if (rawDate.includes('-') && rawDate.split('-')[0].length === 2) {
                   const p = rawDate.split('-'); isoDate = `${p[2]}-${p[1]}-${p[0]}`;
                }
                ...
                const record = {
                  id: Date.now().toString() + Math.random().toString(36).substr(2, 5), date: isoDate,
                  clockIn: parseTime(row['Clock In'] || row['In'] || ''), clockOut: parseTime(row['Clock Out'] || row['Out'] || ''), notes: row['Notes'] || ''
                };
      ...
      function init() { updateDarkModeButton(); lucide.createIcons(); }
